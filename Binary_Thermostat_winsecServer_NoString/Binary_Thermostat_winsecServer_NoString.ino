...
char * dataDump(char * buff, unsigned int len) {
  ...
  sprintf(convert, "],\"Temperature\":%0.2f,\"State\":%d,\"Heat\":%d,\"Cool\":%d,\"Memory\":%d,\"Source\":\"%s\"}", Temperature, Status, Heat, Cool, ESP.getFreeHeap(), Source);
  strcat(buff, convert);
  return buff;
}
...
bool serverRecv() {
  ...
  piServer.readStringUntil('|');
  piServer.readStringUntil(char(5)).toCharArray(bigbuff, 9500);
  if (bigbuff[0] == 0) return false;
  else parse(bigbuff);
}

bool parse(char * cmd) {
   if (strcmp(cmd,"defaultTemps")==0) {
    data.defaultHeat = piServer.parseInt();
    data.defaultCool = piServer.parseInt();
  }
  else if (strcmp(cmd, "schedule")==0) {
    uint8_t pos = piServer.parseInt();
    data.schedule[pos].days = piServer.parseInt();
    data.schedule[pos].heat = piServer.parseInt();
    data.schedule[pos].cool = piServer.parseInt();
    data.schedule[pos].start = piServer.parseInt();
    data.schedule[pos].end = piServer.parseInt();
  } else if (strcmp(cmd,"ScheduleDone")==0) {
    EEPROM_Save();
  } else if (strcmp(cmd,"quickRun")==0) {
    quickRun.heat = piServer.parseInt();
    quickRun.cool =  piServer.parseInt();
    quickRun.time = piServer.parseInt();
    if (quickRun.time != 0) {
...
void loop() {
  if (now() > tempCheck) {
    ...
    while (!(piServer1.connected())) {
      if (piServer1.connect("SERVER", PORT) != 1) {
        break;
      }
  ...
  if (serverCheckin < now() && data.serverUName[0] != 0) {
    if (!piServer.connected()) {
      piServer.stop();
      piServer.connect("SERVER.COM", PORT);
